...
> **Version:** 1.0
> **Last Updated:** 2026-08-05 (**Phase 10 — Security & Compliance complete, all 6 milestones.** Milestone 6 (Final Security Audit): re-verified every §23 checklist item live against real code (25/33 confirmed true, 8 honestly left open with tracked reasons — TD-013/TD-027/TD-028), retracted a factually wrong Milestone 1 finding (F-01 — a password blocklist has existed since Phase 4, confirmed by direct test), corrected a second RBAC documentation error (F-02, plus a related JWT-field inaccuracy found in the same pass), and closed out the phase. See §2 for the full Phase 10 completion summary)
> **Status:** Phase 9 — Subscription & Billing, complete. **Phase 10 — Security & Compliance, complete (6/6 milestones)**; see §2
...
| Property | Value |
|---|---|
| **Current phase** | Phase 9 (Subscription & Billing) complete. **Phase 10 (Security & Compliance) complete, 6/6 milestones** — see §2 |
...
### Complete: Phase 10 — Security & Compliance (6/6 milestones; 9/18 ROADMAP tasks)
...
**Milestone 6 — Final Security Audit (complete):** re-verified every Milestone 2–5 fix live against the running application (F-03/04/05/08/09/10/11 and both GDPR endpoints all reconfirmed working, not assumed to still be intact); re-ran every one of `Security_Architecture.md` §23's 33 checklist items against the real code, not from memory — 25 confirmed true, 8 honestly left unchecked with a specific reason each (2 real, newly-tracked gaps: admin actions and auth events still aren't audit-logged — TD-013/TD-027 — and no Prometheus alert rules exist at all — TD-028; 1 genuinely unverifiable, no live deployment to check Coolify against). While re-verifying F-01, discovered and corrected a real error in this project's **own** Milestone 1 review: the claim "no common-password blocklist exists" was factually wrong — a real ~300-entry blocklist has existed since Phase 4's original commit, confirmed by direct code inspection and a 9-case logic-level test (case-insensitive common-password rejection, length bounds, strong-password acceptance). Retracted rather than left standing or silently "fixed" (there was nothing to fix). Also corrected F-02 (the RBAC role hierarchy doc's unimplemented `viewer` role and incorrectly-nested Super-Admin/Admin hierarchy) and a related error found in the same pass (the JWT field table incorrectly listed `super_admin` as a possible `orgRole` value, when it's always a separate, non-JWT, live-DB-read platform flag — DEC-017). Consolidated the technical debt log: updated TD-013 to confirm its own blocker (F-04) is resolved but its actual wiring remains undone; added TD-027 (admin actions not audit-logged) and TD-028 (no Prometheus alerting) as newly-found gaps; cross-referenced TD-011 and TD-025 with the Milestone 2/5 decisions they shaped. See the 2026-08-05 Status Update History entry below for full detail, including a real Docker Desktop/WSL2 infrastructure outage encountered mid-milestone and how it was worked around.

### Phase 10 Completion Summary

**Not yet merged.** All 6 milestones' work lives on `feat/VS-phase10-security` (7 commits: Milestones 2–6, plus one dependency-fix commit found during Milestone 5's own verification pass). No PR has been opened yet — that happens at the repo owner's direction, matching the same per-phase (not per-milestone) merge pattern Phase 9 used.

**Milestones completed (6/6):**
1. Security Architecture Review — 22-subsystem review, 0 Critical/High findings, 5 Medium/2 Low/3 Informational
2. Application Security Hardening — all 5 confirmed Medium findings fixed and live-verified (F-03/04/05/08/10)
3. API Security & Abuse Protection — found and fixed a real open-redirect vulnerability (F-11); confirmed no SSRF vector exists
4. Infrastructure Security — Docker digest pinning (F-09); found and fixed a real HTTP→HTTPS redirect gap; Dependabot, GitHub Actions SHA-pinning, container hardening
5. Compliance & Privacy — GDPR account export/deletion, 30-day purge job, cookie consent, draft Privacy Policy/ToS; found and fixed a real dependency vulnerability (postcss/sharp via a Next.js bump)
6. Final Security Audit — full regression, complete §23 checklist re-verification, retracted a factually wrong Milestone 1 finding (F-01), corrected F-02, technical debt consolidation

**Major deliverables:** `docs/reviews/security/00-executive-summary.md` through `06-remediation-plan.md`; `GET /api/v1/account/export` + `DELETE /api/v1/account` + the daily privacy-maintenance job; `lib/encryption.ts` (OAuth token encryption); `lib/security-headers.plugin.ts` (Helmet + CSP) and `apps/web`'s nonce-based CSP; the global pre-auth rate limiter; `.github/dependabot.yml`; SHA-pinned GitHub Actions; digest-pinned Dockerfiles; an entrypoint-level HTTP→HTTPS redirect; a fully re-verified `Security_Architecture.md` §23 checklist.

**Security improvements:** closed every Medium-severity finding from the Milestone 1 review (F-03/04/05/08/09/10) plus one found mid-phase (F-11, open redirect) — none speculative, every one reproduced or directly confirmed before being called a finding at all, matching the review's own founding instruction ("only report issues you can justify"). Two genuine dependency vulnerabilities found and fixed via the project's own audit tooling rather than a hypothetical scan (postcss/sharp via Milestone 5's Next.js bump; fast-uri as a side effect of the same). GDPR rights implemented for the first time in this codebase's history. A real documentation error in this project's own prior security review (F-01) was found and corrected rather than left standing.

**Technical debt carried forward:** TD-013 (auth events not audit-logged — its own blocker, F-04, is now resolved, but the wiring itself remains open), TD-027 (admin actions not audit-logged — newly found), TD-028 (no Prometheus alert rules — newly found), TD-025 (API keys still don't authenticate any request — F-05 fixed *who* can manage them, not this), TD-011 (Organisation & Workspace Management — directly shaped two Milestone 5 GDPR design decisions), a genuine gap found in Milestone 5 (no `PATCH` profile endpoint exists despite the security docs having claimed one did — GDPR "right to rectification" is not actually implemented).

**Blockers:** None currently active for the code itself. A real environment issue was encountered and resolved during Milestone 6: the local Docker Desktop/WSL2 backend became unresponsive for one specific container (Redis) mid-audit — plain relaunch, `wsl --shutdown`, and even a GUI force-remove all failed to clear it; a full Windows restart was the fix that worked. Documented here because it delayed, but did not compromise, this milestone's verification — the regression suite had already completed successfully before the outage began.

**Production-readiness assessment:** The security posture is substantially stronger than at Phase 10's start — every Medium finding from the initial review is closed, GDPR compliance exists for the first time, and the audit gate genuinely blocks on unaddressed high/critical CVEs (proven, not assumed: it caught real issues twice this phase). The honest remaining gaps are all explicitly tracked, not hidden: no audit trail for auth/admin events (TD-013/027), no alerting (TD-028), no live-deployment verification of Coolify/production secrets handling (nothing is deployed yet), and the Privacy Policy/Terms of Service are drafts pending real legal review — a step outside what any AI assistant can provide. None of these block a launch on their own merits, but all should be weighed before one.
...
Phase 10         ████████████████████  100%  ✅ Complete (6/6 milestones; 9/18 ROADMAP tasks — see TD-013/025/027/028 for deferred remainder)
...
**Cross-reference (Phase 10):** this gap directly shaped two Milestone 5 GDPR design decisions, worth recording here since they're consequences of this TD, not new problems: (1) `DELETE /api/v1/account` refuses with `409` if the requester solely owns an organisation with other members, rather than silently orphaning them — there's no ownership-transfer flow for it to invoke instead; (2) `GET /api/v1/account/export` is scoped to the requester's single current organisation context, since there's no multi-org membership to loop over yet. Both should be revisited once this TD is resolved.
...
**Update (Phase 10 Milestone 2, 2026-08-01):** The blocker this TD's own resolution needed — `audit_logs`' RLS `WITH CHECK` rejecting the org-less writes a pre-organisation event (a failed login for a nonexistent email, a registration event) would require — is now fixed (migration `0012_audit_logs_null_org_write.sql`, finding F-04). The actual wiring into `register`/`login`/`logout`/`refresh`/`verifyEmail`/`resetPassword`/OAuth callbacks was deliberately **not** done as part of that fix (out of Milestone 2's approved scope) and remains undone — confirmed directly during Phase 10 Milestone 6's final audit (`grep`-checked every call site of `auditLog()` in `apps/api/src`: only `account.service.ts`'s deletion event and `webhook.service.ts`'s billing events call it; `auth.service.ts` still calls it nowhere). This TD's resolution plan below is now fully unblocked and ready to implement whenever picked up.
...
**Cross-reference (Phase 10 Milestone 2, F-05):** a related but distinct gap was fixed — API key *management* (create/revoke) was reachable by any org member, not just owner/admin as documented, independent of whether keys authenticate anything yet. That fix doesn't touch this TD at all (still nothing authenticates a request via an API key), but is worth noting here since both concerns share the same subsystem: this TD is about *using* a key to authenticate a request; F-05 was about *who can create/destroy* a key in the first place.
...
### TD-027 — Admin actions are not written to `audit_logs`

| Property | Value |
|---|---|
| **Logged** | 2026-08-05 |
| **Severity** | Low |
| **Phases affected** | Phase 5 (admin routes built) |
| **Status** | Open |

**Description:** `Security_Architecture.md` §18 documents "Admin actions: User suspended, plan overridden, dead-letter job retried, quota reset, workflow triggered" as an audit-logged category, and §23's Security Checklist requires "All admin actions logged to audit_logs." Confirmed during Phase 10 Milestone 6's final audit (`grep`-checked every call site of `auditLog()`): `admin.routes.ts` never calls it — dead-letter retry, workflow triggers, and every other admin action currently leave no audit trail beyond structured Pino application logs.

**Why not fixed now:** Same category of gap as TD-013 (auth events), found while re-running the security checklist, not while implementing admin routes — wiring this in is separable follow-up work, and Milestone 6 was scoped as an audit pass, not a new-implementation one.

**Resolution plan:** Add `auditLog()` calls to `admin.routes.ts`'s dead-letter retry, workflow-trigger, and any future user-suspension/plan-override actions. Straightforward given `auditLog()` and the `withTenant()` pattern already exist — this is wiring, not design work.

---

### TD-028 — No Prometheus alert rules configured

| Property | Value |
|---|---|
| **Logged** | 2026-08-05 |
| **Severity** | Low |
| **Phases affected** | Phase 2 (Prometheus provisioned) |
| **Status** | Open |

**Description:** `Security_Architecture.md` §23's Security Checklist requires "Prometheus alert configured for auth failure rate spike" and "for anomalous API usage." Confirmed during Phase 10 Milestone 6's final audit: `infra/monitoring/prometheus/prometheus.yml` has no `rule_files`/`alerting` section at all, and no separate alert-rules file exists anywhere in the repo — Prometheus is provisioned and scraping itself, but zero alert rules of any kind are configured, security-related or otherwise.

**Why not fixed now:** Defining a *good* alert rule (the right threshold, the right query, avoiding alert fatigue from a too-sensitive rule) needs real traffic-pattern data this environment doesn't have (nothing is deployed yet — same gating as TD-001/TD-006). Writing a rule now would be guessing thresholds, not configuring a considered one.

**Resolution plan:** Once real traffic data exists (post-launch or from a staging environment under load), add a `rule_files` section to `prometheus.yml` with at minimum: a rate-based alert on `401`/`429` responses from `/api/v1/auth/*` (auth failure spike) and a per-IP or per-API-key request-rate anomaly rule. Related to TD-002 (no distributed tracing) — both are pieces of the same "real production observability isn't built yet" gap.
