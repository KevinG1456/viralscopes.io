 ...
 *
 * Postgres's RETURNING clause omits rows skipped by ON CONFLICT DO NOTHING,
 * so a plain `.returning()` would be undefined on every re-run against an
 * already-seeded database -- the fallback SELECT is what makes this
 * idempotent across repeated `npm run db:seed` invocations.
 */
